...
const CorteCaja = ({ onClose }: CorteCajaProps) => {
  ...
  useEffect(() => {
    ...
    if (filtroVendedor !== null) {
      resultado = resultado.filter(venta => 
        venta.usuario && venta.usuario.id === filtroVendedor
      );
    }
  ...
  const calcularNumeroVentas = () => {
    return ventasFiltradas.filter(venta => !venta.venta.anulada).length;
  };
...
/*   // Imprimir corte de caja
  const imprimirCorte = async () => {
    ...
    try {
      // Aquí iría la lógica para imprimir el corte de caja
      // Por ejemplo, usando una API para enviar a una impresora
      await new Promise(resolve => setTimeout(resolve, 1500)); // Simulación
      
      toast.success('Corte de caja impreso correctamente');
    } catch (error) {
      console.error('Error al imprimir corte:', error);
      toast.error('No se pudo imprimir el corte de caja');
    } finally {
...
/*   // Exportar corte a Excel
  const exportarExcel = async () => {
    try {
      const blob = await VentaService.descargarReporte(fechaHoy, fechaHoy, 'excel');
      
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `corte-caja-${format(new Date(), 'yyyy-MM-dd')}.xlsx`;
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              value={filtroVendedor !== null ? filtroVendedor : ''}
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {ventasFiltradas.map((venta) => (
                      <tr key={venta.venta.ventaId} className={venta.venta.anulada ? 'text-gray-400 dark:text-gray-500' : ''}>
                        ...
                        <td className="px-4 py-2 whitespace-nowrap text-sm">
                          {new Date(venta.venta.fecha).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </td>
                        <td className="px-4 py-2 whitespace-nowrap text-sm">
